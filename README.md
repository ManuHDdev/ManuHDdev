<div align="center">
  
# 👋 Hola, soy Manuel Horrillo

### Desarrollador Full Stack | Angular + Spring Boot

<p>
  <a href="https://manushd.github.io/Curriculum/"><img src="https://img.shields.io/badge/CV-255E63?style=for-the-badge&logo=About.me&logoColor=white"/></a>
  <a href="https://www.linkedin.com/in/manuel-horrillo-diestro-52015a373/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:manuhddev@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
</p>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&pause=1000&color=6366F1&center=true&vCenter=true&width=435&lines=Ingeniero+Software;Full+Stack+Developer;Arquitecturas+Empresariales;Aprendiendo+cada+d%C3%ADa+%F0%9F%9A%80" alt="Typing SVG" />

</div>

---

## 🚀 Sobre mí

Ingeniero Informático con experiencia real desarrollando software. Me gustaría destacar que trabajé profesionalmente desarrollando un **sistema integral de gestión logística** para el distribuidor oficial de **Chanel en España**.

- 🔭 Actualmente trabajando en Sistema WMS para Chanel España
- 🌱 Aprendiendo continuamente sobre arquitecturas de microservicios y buenas prácticas
- 💼 Desarrollador Full Stack en DELIM (Distribuidor Oficial Chanel España)
- 🎯 Especializado en **Angular**, **Spring Boot** y **arquitecturas empresariales**
- 📍 Badajoz, España
- ⚡ Dato curioso: Transformé procesos manuales de Excel a un sistema robusto que gestiona +10,000 referencias, ubicaciones y sus respectivos estados

---

## 🛠️ Stack Tecnológico

### Frontend
<p>
  <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white"/>
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/RxJS-B7178C?style=for-the-badge&logo=reactivex&logoColor=white"/>
</p>

### Backend
<p>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white"/>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white"/>
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white"/>
</p>

### Base de Datos
<p>
  <img src="https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white"/>
  <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white"/>
</p>

### DevOps & Herramientas
<p>
  <img src="https://img.shields.io/badge/Docker-2CA5E0?style=for-the-badge&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white"/>
  <img src="https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white"/>
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"/>
</p>

### Seguridad & Autenticación
<p>
  <img src="https://img.shields.io/badge/Keycloak-008AAA?style=for-the-badge&logo=keycloak&logoColor=white"/>
  <img src="https://img.shields.io/badge/OAuth2-000000?style=for-the-badge&logo=auth0&logoColor=white"/>
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=white"/>
</p>

---

## 💼 Experiencia Destacada

### 🏢 Desarrollador Full Stack @ DELIM (Chanel España)
**Nov 2024 - Actualidad** | Madrid (Híbrido)

Responsable del desarrollo completo de un **sistema WMS empresarial** que actualmente gestiona el control logístico de todos los productos Chanel distribuidos en España.

**Logros clave:**
- ✅ Sistema en **producción** gestionando +10,000 referencias de productos
- ✅ Arquitectura de **12 microservicios** independientes y escalables
- ✅ Reducción del **70%** en tiempo de entrada de datos mediante validaciones inteligentes
- ✅ Mejora del **100%** en trazabilidad del inventario
- ✅ Gestión completa de **infraestructura**: Ubuntu Server, Nginx, SSL, MySQL, backups automatizados

**Stack:** Angular 16 • Spring Boot 17 • Keycloak • MySQL • Docker • OAuth2 • JWT

---

## 🚀 Proyectos Destacados

### 🏪 [Sistema WMS - Chanel España](https://github.com/ManuHDdev)
Sistema integral de gestión de inventario empresarial con arquitectura de microservicios. Actualmente en producción gestionando toda la logística de Chanel en España.

**Características:**
- 12 microservicios independientes
- Autenticación empresarial con Keycloak (OAuth2 + OpenID Connect)
- +120 funcionalidades implementadas
- Importación/exportación masiva de datos
- Sistema de roles y permisos granulares
- Generación automática de documentos (PDFs, etiquetas)
- Backups automatizados con Google Drive

**Tech Stack:** `Angular 16` `Spring Boot 17` `Keycloak` `MySQL` `PostgreSQL` `Docker` `Nginx`

---

### ⚽ [Bobble Premier League](https://bobblepremierleagueoficial.web.app)
Plataforma web para comunidad gaming con gestión completa de estadísticas, clasificaciones, partidos y torneos.

**Tech Stack:** `Angular` `TypeScript` `Google Sheets API` `Firebase`

---

### 👴 [Centro de Mayores Coexate](https://coexate.web.app)
Sitio web informativo diseñado con enfoque en accesibilidad para personas mayores.

**Tech Stack:** `HTML5` `CSS3` `JavaScript` `Firebase Hosting`

---

## 📈 Actividad Reciente

<!--START_SECTION:activity-->
<!--END_SECTION:activity-->

---

## 📫 ¿Hablamos?

<div align="center">

**¿Tienes un proyecto en mente? ¿Buscas colaborar?**

📧 **Email:** manuhddev@gmail.com  
💼 **LinkedIn:** [Manuel Horrillo Diestro](https://www.linkedin.com/in/manuel-horrillo-diestro-52015a373/)  
🌐 **Portafolio:** [manushd.github.io/Curriculum](https://manuhddev.github.io/Portafolio/)  
📍 **Ubicación:** Badajoz, España

</div>

---

<div align="center">
  
### 💡 "El código es poesía en movimiento"

![Profile Views](https://komarev.com/ghpvc/?username=ManuHDdev&color=6366f1&style=for-the-badge)

**⭐ Si te gusta mi trabajo, no dudes en dejar una estrella en mis repos**

</div>
